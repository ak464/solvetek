...
export function ArticleCard({ article, className, variant = "default" }: ArticleCardProps) {
    ...
    if (isPoster || isCompactPoster) {
        return (
            <Link
                ...
                className={cn(
                    "group relative block bg-primary overflow-hidden transition-all duration-500 rounded-3xl border-4 border-border shadow-2xl",
            ...
            >
                ...
                <div className="absolute inset-0 z-20 p-6 flex flex-col justify-end text-right">
                    <h3 className={cn(
                        ...
                        isPoster ? "text-xl md:text-3xl" : "text-base md:text-lg decoration-primary/50"
                    )}>
                        {article.title}
                    </h3>
                    {isPoster && (
    ...
    return (
        <div className={cn(
            "group block bg-card border border-border hover:border-primary/50 transition-all duration-500 rounded-xl overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-primary/10 flex flex-col",
            className
        )}>
            ...
            <div className="p-4 flex flex-col justify-between flex-1">
                <Link href={`/guides/${article.category?.slug}/${article.slug}`} className="block">
                    <h3 className="font-black text-primary group-hover:opacity-80 transition-colors line-clamp-2 text-sm md:text-base font-heading leading-snug mb-2">
                        {article.title}
                    </h3>
